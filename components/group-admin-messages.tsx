...
export function GroupAdminMessages({ groupId }: { groupId: string }) {
  ...
  React.useEffect(() => {
    getConversationsForGroup(groupId).then((list) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Conversations started by members who clicked &quot;Message organizer&quot; on the group page. Reply here or open in the messenger widget.
      </p>
      {loading ? (
          ...
          <ul className="border rounded-2xl divide-y max-h-[50vh] overflow-y-auto">
            {conversations.map((c) => (
              <li key={c.id}>
                ...
                >
                  <p className="font-medium text-sm">{c.member_name ?? "Someone"}</p>
